...
//translate([50,-35,-3.5]) rotate([90,0,90]) bandPost();
	
//Post to stop right bolt from pulling back when unscrewing from cube
//rightBoltRearStop();
//rightBoltRearStopExtended();

//Cutout with hook for rubber band - to be differenced into an object
//width/depth/height are dimensions for the cutout, and the hook is scaled to fit
...
//microscope slide to set z-position of fiber
//color("lightBlue") microscopeSlide();


module rightBoltRearStopExtended(){
	difference(){
		translate([-25,-50,-11])
			difference(){
				union(){
		 ...
		 translate([cubeSize/2,-threadLengths[0]-8,0])
			rotate([0,90,90])
				cylinder(d=m3Cutout, h=50, center=true, $fn=50);  
		//Left bolt cutout	
...
module bandHookInv(width, depth, height, $fn=30){
	difference(){
		cube([width,depth,height]);
		translate([width/1.7+0.01,0,height/2])
			union(){
				translate([0,depth*0.1,0])
					sphere(d=min(width,height)*0.6);
				rotate([0,90,0])
					cylinder(h=width/2,d=min(width,height)/1.8);
				}
	}
}
...
module gearFrame($fn=50){
	difference(){
		union(){
			...
			//Rubber band holding post - right underside
			translate([cubeSize/2,-threadLengths[0]/2-10,-11])
				cylinder(d=4,h=10);
		...
		translate([-threadLengths[1]-2.5,cubeSize/2,0])
			rotate([0,-90,0])
				cylinder(d=m3Cutout, h=40, center=true);
		//2nd
		...
		translate([cubeSize/2,-threadLengths[0]-8,0])
			rotate([0,90,90])
				cylinder(d=m3Cutout, h=50, center=true);
		...
		translate([39,-threadLengths[0]+9.0,-3.5])
		 ...
		 //Cutouts for awkward rubber band
		 for (i=[0:1]){
			 translate([(2*i+1)*cubeSize/4,-threadLengths[0]/2-5,1])
				rotate([0,-90,90])
					slot(3,10,10);
					...
					union(){
						...
						translate([0,0,0])
							...
							linear_extrude(3,scale=1/((2*3*tan(45)/gearDiam)+1))
								#ourGear(intNumTeeth+11,0,0, true);
...
module mechBody(){
    ...
    //Hole in rear cube support plate for bolt to reduce cube position slop
    translate([-5,20,0])
		rotate([0,90,90])
			cylinder(d=2.8, h=20, center=true, $fn=50);    
...
module cubes(sliders=true, sliderSelection=[0,1], driven=true, drivenSelection=[0,1]){
	...
	for (i=sliderSelection){
		...
		difference(){
			...
			//Differences between cubes:
			//One has cylinder cutout for mounting bolt to pass through,
			//  other has trylinder for rubber band bolt to thread into.
			//Second cube has U shaped cutout for band to be inserted into.
			if (i==0){
				//Passthrough hole for cube mounting bolt
				cylinder(d=3.2, h=cubeHeight+1, center=true, $fn=50);
			}
			else{
				//Self tap hole for rubber band holding bolt
				trylinder_selftap(h=11, center=true, $fn=30);
				
				//U cutout for rubber band
				translate([0,2,-2.5])
					rotate([0,0,90])
						slot(cubeSize-7,30,5);
	...
	if (driven==true){
		...
		difference(){
			...
			//The +0.4*i is to make the nut slot in one of the driven cubes slightly taller, as the different print orientation causes slight collapsing of the slot
			 ...
			 //Rubber band holding hooks
			 if (i==1){
				 translate([8,10.1,-3.5])
					rotate([0,0,180])
						bandHookInv(5,5,7);
				 translate([-8,10.1,3.5])
					rotate([0,180,180])
