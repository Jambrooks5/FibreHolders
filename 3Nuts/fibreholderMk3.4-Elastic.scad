...
module frameCorner(){
    translate([-threadLengths[0]+3,-threadLengths[0]-13,-11])
        rotate([0,0,-45])
            lSection(15,5,15,5,15);
}
...
module gearFrame($fn=50){
    difference(){
        union(){
            ...
            translate([-threadLengths[0]-10,threadLengths[0]-14.5,-11])
                cube([10,10,15]);
        ...
        //Mounting bolt holes
        translate([-threadLengths[0]-6,30,-3.5])
            rotate([90,-90,180])
...
module gearPairs(){
    ...
    if (gearSelection[1]==1){
        ...
        //Intermediate
        translate([-threadLengths[0]+5,cubeSize/2-gearDiam/2-intGearDiam/2-gearTol,0])
            rotate([-90,180/intNumTeeth,-90])
                difference(){
                    union(){
                        ourGear(intNumTeeth,3,5);
                        translate([0,0,0])
                            rotate([0,180,0])
                            linear_extrude(3,scale=(2*3*tan(45)/gearDiam)+1)
                                #ourGear(intNumTeeth+9,0,0, true);
...
module movingBolts(){
    ...
    translate([cubeSize/2,-threadLengths[0]-14,0])
        rotate([90,0,0])
